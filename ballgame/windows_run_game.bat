...
:start_update_screen
if exist "%SCRIPT_DIR%Contents\code\Saves\Update_screen.py" (
    start /b python "%SCRIPT_DIR%Contents\code\Saves\Update_screen.py"
    set UPDATE_SCREEN_PID=%!
) else (
    echo Contents\code\Saves\Update_screen.py not found. Please check the path and try again.
    exit /b 1
)
...
if exist "%SCRIPT_DIR%Contents\code\Main.py" (
    python "%SCRIPT_DIR%Contents\code\Main.py"
) else (
